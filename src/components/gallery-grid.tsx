...
import { Eye, Heart, ExternalLink, X } from 'lucide-solid'
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredItems().map((item, index) => (
            <div 
              key={item.id}
              class="animate-on-scroll group cursor-pointer"
              onClick={() => setSelectedImage(item.id)}
            >
              <div class="relative aspect-square rounded-lg overflow-hidden glow-accent">
                <img
                  src={item.image}
                  alt={item.alt}
                  class="object-cover transition-transform duration-500 group-hover:scale-110 absolute inset-0 w-full h-full"
                />
                
                {/* Overlay */}
                <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <div class="text-center text-white p-4">
                    <Eye class="h-6 w-6 mx-auto mb-2" />
                    <h3 class="font-caps text-sm mb-1">{item.title}</h3>
                    <p class="text-xs opacity-80">{item.venue}</p>
                  </div>
                </div>

                {/* Category Badge */}
                <div class="absolute top-3 left-3">
                  <span class="bg-accent text-accent-foreground px-2 py-1 rounded text-xs font-caps capitalize">
                    {item.category}
                  </span>
                </div>
        ...
        {filteredItems().length > 0 && (
          <div class="text-center mt-12 animate-on-scroll">
            <button class="bg-card/50 backdrop-blur-sm border border-border rounded-lg px-8 py-3 font-caps text-sm hover:glow-accent transition-all duration-300">
              Load More Images
            </button>
          </div>
        )}
